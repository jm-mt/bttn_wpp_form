...
(function() {
    ...
    const initWidget = () => {
        ...
        const state = {
            ...
            isProcessing: false,
            privacyAccepted: false,
            preOpenedWindow: null,  // Janela pre-aberta para evitar bloqueio de popup
            timers: {
        ...
        const handleUserInput = async () => {
            ...
            if (fieldType === 'phone') {
                ...
                // PRE-ABRE a janela do WhatsApp agora (no clique do usuario)
                // Isso evita bloqueio de popup no redirecionamento automatico
                preOpenWhatsAppWindow();
        ...
        // Pre-abre uma janela em branco no momento do clique do usuario
        // Isso evita bloqueio de popup porque a janela e aberta em uma acao do usuario
        const preOpenWhatsAppWindow = () => {
            try {
                // Abre janela em branco
                state.preOpenedWindow = window.open('about:blank', '_blank');

                if (state.preOpenedWindow) {
                    // Escreve conteudo de loading na janela
                    state.preOpenedWindow.document.write(`
                        <!DOCTYPE html>
                        <html>
                        <head>
                    ...
                    `);
                    state.preOpenedWindow.document.close();
                    log('Pre-opened WhatsApp window successfully');
                }
            } catch (e) {
                log('Failed to pre-open window:', e);
                state.preOpenedWindow = null;
            }
        };
        ...
        const goToWhatsApp = (type = 'app') => {
            ...
            // Usa a janela pre-aberta se disponivel
            if (state.preOpenedWindow && !state.preOpenedWindow.closed) {
                state.preOpenedWindow.location.href = url;
                log('Redirected using pre-opened window');
                state.preOpenedWindow = null;
                return;
            }

            // Fallback: tenta abrir nova aba
            const newWindow = window.open(url, '_blank', 'noopener,noreferrer');

            // Se bloqueado, redireciona na mesma aba
            if (!newWindow || newWindow.closed || typeof newWindow.closed === 'undefined') {
                log('Popup blocked, redirecting in same tab');
        ...
        const showWhatsAppChoiceMessage = () => {
            ...
            messageEl.innerHTML = `
                <div class="wwl-bubble wwl-bubble-interactive">
                    <div class="wwl-bubble-text">
                        Onde voce prefere continuar a conversa?
                    </div>
                    <div class="wwl-bubble-buttons">
                        <button type="button" class="wwl-bubble-btn" data-type="web">
                            <svg viewBox="0 0 24 24" width="18" height="18">
                                <path fill="currentColor" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/>
                            </svg>
                            WhatsApp Web
                        </button>
                        <button type="button" class="wwl-bubble-btn wwl-bubble-btn-primary" data-type="app">
                            <svg viewBox="0 0 24 24" width="18" height="18">
                    ...
                    <div class="wwl-bubble-footer">
                        <span class="wwl-bubble-time">${getCurrentTime()}</span>
                        <span class="wwl-bubble-countdown">Abrindo em <strong>${countdown}s</strong></span>
                    </div>
                </div>
            `;
            ...
            buttons.forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    if (countdownContainer) {
                        countdownContainer.innerHTML = '<span style="color: #25D366;">Redirecionando...</span>';
                    }

                    // Marca o botao como clicado
                    btn.classList.add('wwl-clicked');

                    // Redireciona usando a janela pre-aberta
                    const type = btn.dataset.type;
                    goToWhatsApp(type);

                    log('WhatsApp redirect via button:', type);
                });
            });
            ...
            autoRedirectTimer = setInterval(() => {
                ...
                if (countdown <= 0) {
                    ...
                    // Redireciona automaticamente usando janela pre-aberta
                    goToWhatsApp('app');
            ...
            // Salva referencia do timer no state para poder cancelar se necessario
