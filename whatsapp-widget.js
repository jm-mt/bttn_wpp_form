...
(function() {
    ...
    const initWidget = () => {
        ...
        const goToWhatsApp = (type = 'app', viaLink = false) => {
            ...
            // Se foi via link (clique do usuario), o proprio link abre
            // Se foi automatico, usa window.open ou location
            if (!viaLink) {
        ...
        const showWhatsAppChoiceMessage = () => {
            ...
            // Event listeners nos botoes (para tracking)
            const buttons = messageEl.querySelectorAll('.wwl-bubble-btn');
            buttons.forEach(btn => {
                btn.addEventListener('click', (e) => {
                    // Para o timer quando usuario clica
                    ...
                    pushToDataLayer(CONFIG.tracking.gtm.events.redirected, {
                        leadId: state.leadData.id,
                        redirectType: btn.dataset.type,
                        method: 'button_click'
                    });

                    log('WhatsApp redirect via button:', btn.dataset.type);
                });
            });

            // Auto-redirect apos 4 segundos
